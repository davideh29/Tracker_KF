...
def load_testing_data(binary_masks_path, color_path):
    ...
    # Loop through images
    for i in range(0, len(bin_mask_files)):
        # Load color image
        # Load segmentation image
        bin_mask.append(cv2.imread(binary_masks_path + format(bin_mask_files[i]) + '.png', 0))
        vid_img.append(cv2.imread(color_path + format(vid_img_files[i]) + '.tif', 1))

    # Convert to ndarray and return
    bin_mask = np.array(bin_mask)
    vid_img = np.array(vid_img)
    return bin_mask, vid_img


def gen_features(bin_mask, min_area):
    # Output vector of features
    feat_vect = []
    contour_vect = []
    centroid_vect = []
    all_feats = []
    for i in range(0, len(bin_mask)):
        # Segmentation image
        bw_img = bin_mask[i]
        # Find contours
...
def contour_feats(contour):
    img_feat = np.empty(14)
    M = cv2.moments(contour)
    # Centroid
    centroid = np.array([M['m10'] / M['m00'], M['m01'] / M['m00']])
    # Area
    img_feat[0] = cv2.contourArea(contour)
    # Perimeter
    img_feat[1] = cv2.arcLength(contour, True)
    ...
    img_feat[4] = dist.max()
    # Mean distance
    img_feat[5] = dist.mean()
    img_feat[6] = dist.std()
    img_feat[7:14] = cv2.HuMoments(M).flatten()
